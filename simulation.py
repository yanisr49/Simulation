...
if __name__ == "__main__":
    best_eval = 0
    best_values = []
    for nb_conseille_total in range(10, 20):
        for nb_poste in range(8, nb_conseille_total):
            for nb_conseille_appel in range(nb_poste):
                eval = 0
                for i in range(10):
                    ...
                    (1 - (10 * (config.aire_qt/(240 * (config.nb_appel_traite + config.qt))))) + \
                    ((config.aire_ct + config.aire_cm) / (240 * config.n)) + \
                    (config.aire_ct / (240 * config.nt_max))
                if eval > best_eval:
                    best_eval = eval
                    best_values = [nb_conseille_total, nb_conseille_appel, nb_poste]
    print("Nombre conseillé total : " + str(best_values[0]))
    print("Nombre conseillé appel : " + str(best_values[1]))
    print("Nombre poste : " + str(best_values[2]))
